...
import { allServicesData } from "../data/ServicesData"; // Make sure your data is exported from here
...
const ScrollNav = ({ sections, activeSection, scrollToSection, isMobileVisible }) => {
  // 1. Find the active index to calculate position
  const activeIndex = sections.findIndex((s) => s.id === activeSection);

  // 2. Define spacing constants (approximate based on your CSS classes)
  // Icon height (48px) + Text/Margin buffer + space-y-12 (48px)
  // You may need to tweak this 'ITEM_HEIGHT' number slightly to match your exact font rendering
  ...
  return (
    <div className={`hidden lg:block sticky top-32 space-y-4 transition-opacity duration-300 ${isMobileVisible ? 'opacity-100' : 'opacity-100'}`}>
      <div className="relative pl-4 sm:pl-6 border-l-2 border-slate-800 space-y-8 sm:space-y-12">
        ...
        <div
          ...
          style={{
            // FIX: Set a fixed height (e.g., 48px matches your h-12 icon)
            height: '48px',
            // FIX: Move top position based on the active index * spacing
            top: `${activeIndex * ITEM_HEIGHT}px`,
            // Optional: Hide if no section is active
        ...
        {sections.map((section) => {
          ...
          return (
            <button
              ...
              // Added h-[48px] to container to ensure consistent height for math
              className={`group flex flex-col items-start text-left transition-all duration-300 ${isActive ? 'scale-105' : 'opacity-60 hover:opacity-100'}`}
            >
              <div
                className={`
...
const MobileNav = ({ sections, activeSection, scrollToSection, isOpen, onClose }) => (
  <>
    ...
    {/* Drawer */}
    <div className={`
      fixed left-0 top-0 h-full w-64 bg-gradient-to-b from-[#020617] to-[#0a0f1d] border-r border-slate-800 z-50 lg:hidden
    ...
    `}>
      ...
      <div className="pt-16 px-4 space-y-3">
        <div className="mb-4">
          <span className="text-xs font-bold tracking-widest text-cyan-400 uppercase">Service Sections</span>
        </div>
        {sections.map((section) => {
          const isActive = activeSection === section.id;
          return (
...
const Services = () => {
  // 1. DYNAMIC ID: Get the slug from the URL
  ...
  // 2. DATA LOOKUP: Find the correct service object
  const currentService = useMemo(() => {
    if (!slug) return allServicesData[0]; // Default fallback
    return allServicesData.find(s => s.slug === slug || s.id === slug) || null;
  }, [slug]);

  // Set initial active section when service changes
  useEffect(() => {
  ...
  // Scroll Spy Logic with better position tracking
  useEffect(() => {
    const container = containerRef.current;
    if (!container || !currentService?.sections) return;

    const handleScroll = () => {
      // Show mobile nav when scrolling
      setIsScrolling(true);
      ...
      // Hide mobile nav after scrolling stops
        ...
        <section className="relative py-12 sm:py-20 md:py-24 bg-gradient-to-b from-[#050b1f] to-[#020617] border-t border-slate-800/50">
           <div className="max-w-7xl mx-auto px-4 sm:px-6">
              <div className="text-center mb-12 sm:mb-16 animate-slide-up">
                 <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-white mb-4">Why Choose 1TechHub?</h2>
